...
function Initialize-AzureSubscription {
    ...
    if ($Endpoint.Auth.Scheme -eq 'Certificate') {
        ...
        Set-AzureSubscription -SubscriptionName $Endpoint.Data.SubscriptionName -SubscriptionId $Endpoint.Data.SubscriptionId -Certificate $x509Cert @additional
        Set-CurrentAzureSubscription -SubscriptionId $Endpoint.Data.SubscriptionId # TODO: Should storage account be passed?
        ...
        # TODO: Would Add-Azure(RM)Account generate an error? If so, the next IF block can be omitted altogether in the new handler.
        ...
        # TODO: If Add-Azure(RM)Account generates an error on failure, then this should move into the approriate IF block above.
        if ($azureAccount) {
    ...
    try {
        foreach ($programFiles in @(${env:ProgramFiles(x86)}, $env:ProgramFiles)) {
            ...
            if (Test-Path -LiteralPath $path -PathType Leaf) {
                ...
                Import-Module -Name $azureFolder -Global -PassThru -Verbose:$false
                return
            }
        }
