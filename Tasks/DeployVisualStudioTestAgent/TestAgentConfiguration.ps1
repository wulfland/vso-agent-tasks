...
function InvokeDTAExecHostExe([string] $Version, [System.Management.Automation.PSCredential] $MachineCredential)
{
    ...
    {
        throw (Get-LocalizedString -Key "Could not locate TestAgent installation directory for `$Version={0}. Ensure that TestAgent is installed." -f $Version)
    }
...
function InvokeTestAgentConfigExe([string[]] $Arguments, [string] $Version, [System.Management.Automation.PSCredential] $UserCredential)
{
    ...
    throw (Get-LocalizedString -Key "Did not find TestAgentConfig.exe at : {0}. Ensure that TestAgent is installed." -f $exePath)
}
